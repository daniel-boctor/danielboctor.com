        ...
        if ($.isEmptyObject(data) === true) {
            ...
            for (let [name, info] of Object.entries(data)) {
                ...
                <tbody><tr>
                    ...
                    <td><a href="/read/${name}" class="link-primary">Populate</a></td></tr>
...
function order_trades() {
    ...
    if (["CAD", "USD"].includes(value)) {document.querySelector("#initial-type").innerHTML = "$"} else {document.querySelector("#initial-type").innerHTML = "#"}
    if (document.querySelector("#td_cad").parentElement.childNodes[1].id.substring(3, 7).toUpperCase() != document.querySelector("#id_initial_fx").value) {
        if (document.querySelector("#td_cad").parentElement.childNodes[1].childNodes[2].id.split('_')[1]  != document.querySelector("#id_initial_fx").value) {
